...
#
# Numerical:
#   manually entered value -> numeric parsing -> SAW range lookup first
#   -> hard-cutoff fallback if no SAW match -> PASS / ELIMINATE
#
# Qualitative:
#   manually selected reference text -> normalized exact SAW-text lookup first
#   -> hard-cutoff text fallback if no SAW match -> PASS / ELIMINATE
#
# No fuzzy matching and no automatic unit conversion are used.
# ---------------------------------------------------------------------------
...
def _unicode_superscript_to_ascii(text: str) -> str:
    table = str.maketrans({
        "⁰":"0", "¹":"1", "²":"2", "³":"3", "⁴":"4",
        "⁵":"5", "⁶":"6", "⁷":"7", "⁸":"8", "⁹":"9",
        "⁻":"-", "⁺":"+", "⁽":"(", "⁾":")",
...
def parse_number_strict(x: Any) -> Optional[float]:
    """
    Parse a genuinely numerical reference/input value without fuzzy matching.

    Supported forms include:
      20, 20.5, 1e-6, 1*10^-6, 1×10⁻⁶, 1 x 10^-6
      and numeric values followed by units such as '20 mD' or '10%'.

    Leading comparison operators are accepted for reference range endpoints
    (e.g. '>100', '<1*10^-9', '≥20') and the operator itself is ignored here;
    parse_operator_cutoff()/parse_saw_score_numeric() interpret the operator.
    """
    if is_missing(x):
        return None
    ...
    s = str(x).strip()
    s = _unicode_superscript_to_ascii(s)
    s = s.replace("×", "*").replace("·", "*")
    s = re.sub(r"(?i)\bten\s*\^\s*", "10^", s)
    s = re.sub(r"(?i)\bx\s*10", "*10", s)
    # Unicode superscripts such as 10⁻⁹ become 10-9 after translation;
    # restore the exponent marker so scientific notation is evaluated correctly.
    s = re.sub(r"(?i)(10)([+-]\d+)", r"\1^\2", s)
    s = s.replace("≥", ">=").replace("≤", "<=")
    s = clean_number_str(s)
    s = re.sub(r"^\s*(>=|<=|>|<|=)\s*", "", s)
    s = s.strip()

    # Scientific notation written as coefficient * 10^exponent.
    sci = re.match(
    ...
    if sci:
        try:
            v = float(sci.group(1)) * (10.0 ** int(sci.group(2)))
            return v if math.isfinite(v) else None
        except (OverflowError, ValueError):
            return None

    # Standard scientific/decimal number at the start of the cell.
    m = re.match(
        r"^\s*([+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)",
        s,
    )
    ...
    # Only allow whitespace, %, or a unit/type suffix after the number.
    remainder = s[m.end():].strip()
    if remainder and not re.match(r"^[%a-zA-Zµμ/.*^0-9_\- ]*$", remainder):
        return None
    try:
        v = float(m.group(1))
        return v if math.isfinite(v) else None
...
def parse_operator_cutoff(value: Any) -> Tuple[Optional[str], Optional[float]]:
    ...
    s = _unicode_superscript_to_ascii(str(value).strip())
    s = s.replace("≥", ">=").replace("≤", "<=").replace("×", "*").replace("·", "*")
    s = clean_number_str(s)
    m = re.match(r"^\s*(>=|<=|>|<|=)\s*(.+?)\s*$", s)
    if m:
        number = parse_number_strict(m.group(2))
...
def parse_saw_score_numeric(
    ...
    """Match a numeric value against the five reference SAW ranges.

    Handles normal closed ranges (20–35), exact values (1), and open-ended
    reference cells such as '>100' or '<1*10^-9'. Boundary values are
    inclusive for closed ranges; explicit > / < operators remain strict.
    """
    for start, end, score in ranges:
        start_op, start_num = parse_operator_cutoff(start)
        end_op, end_num = parse_operator_cutoff(end)

        if start_num is None and end_num is None:
            continue

        # Explicit open lower bound, e.g. >100.
        if start_op in (">", ">=") and start_num is not None and end is None:
            if numeric_cutoff_pass(field_value, start_num, start_op):
                return score
            continue

        # Explicit open upper bound, e.g. <1e-9.
        if start_op in ("<", "<=") and start_num is not None and end is None:
            if numeric_cutoff_pass(field_value, start_num, start_op):
                return score
            continue
        ...
        # Single exact value.
        if end is None and start_num is not None and start_op in (None, ">", ">="):
            if math.isclose(field_value, start_num, rel_tol=1e-9, abs_tol=1e-12):
                return score
            continue

        # Closed range. Reference sheets sometimes list high-to-low values;
        # normalize the endpoints before comparing.
    ...
    for cat in CATEGORIES:
        ...
        for spec in reference["categories"][cat]:
            ...
            if not result["pass"]:
                ...
                # A hard-cutoff-only PASS intentionally has no SAW score.
                # Do not add None and do not manufacture a score of 0.
                weighted_sum += float(result["weighted_score"])

        # Only parameters with an actual reference SAW match contribute to
        # the category SAW total. Hard-cutoff-only PASS parameters contribute
        # no score, exactly as defined by the framework methodology.
