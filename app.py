...
def render_manual_field_editor(field_name: str, reference: dict[str, Any]):
    ...
    st.divider()
    st.subheader("Phase-2 field economic data")
    st.caption(
        "These are the field's actual CAPEX and OPEX values. They are stored with the field and later compared against the user's expected CAPEX/OPEX in Phase 2."
    )
    ...
    with ec1:
        state["actual_capex"] = st.text_input(
            ...
            placeholder="Enter actual CAPEX or N/A / Nil / -",
        )
    with ec2:
        state["actual_opex"] = st.text_input(
            ...
            placeholder="Enter actual OPEX or N/A / Nil / -",
        )
...
with st.sidebar:
    ...
    st.caption("• Case differences are irrelevant to the underlying engine.")
    st.caption("• Numerical values must satisfy the hard gate AND fall inside a SAW range.")
    st.caption("• Qualitative values are matched against the complete reference SAW vocabulary.")
...
if phase == "Phase 1 — Screening":
    ...
    if st.button(
        ...
        with st.spinner("Normalizing field inputs, applying hard cut-offs, calculating SAW scores and ranking fields..."):
            passed = []
            failed = []
            details = {}
